CREATE POLICY "Trip members can replace own uploads"
ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id = 'trip-photos' AND is_trip_member(((storage.foldername(name))[1])::uuid))
WITH CHECK (bucket_id = 'trip-photos' AND is_trip_member(((storage.foldername(name))[1])::uuid));

CREATE OR REPLACE FUNCTION public.save_trip_expense(p_trip_id uuid, p_title text, p_amount numeric, p_paid_by uuid, p_member_ids uuid[], p_expense_id uuid DEFAULT NULL::uuid, p_receipt_path text DEFAULT NULL::text, p_request_id uuid DEFAULT NULL::uuid, p_clear_receipt boolean DEFAULT false)
 RETURNS uuid
 LANGUAGE plpgsql
  ...
  ELSE
    UPDATE public.trip_expenses
       SET title = p_title,
           amount = p_amount,
           paid_by = p_paid_by,
           receipt_path = CASE WHEN p_clear_receipt THEN NULL ELSE COALESCE(p_receipt_path, receipt_path) END
